function [output] = PortefeuilleOptimalisatie(doel,pars,restricties,Niter,Nport,tekst)

schaal = 3; %Zorgt voor een grotere nauwkeurigheid -> TODO: waarom niet in master sheet? (sheet "Optimalisatie")
Nport  = Nport*schaal;
N      = size(pars.meanmat,1); 
w_0    = ones(N,1)/N;           % begin equal weight
...
[U_i,m_i,s_i]  = Utility(w_i,doel,pars,restricties,restricties.Ax,restricties.onx);
...
for i=1:Niter
    ...
    [U_j,m_j,s_j] = Utility(w_j,doel,pars,restricties,restricties.Ax,restricties.onx);
...
function [U,re,te] = Utility(ww,doel,pars,restricties,Ax,onx)
