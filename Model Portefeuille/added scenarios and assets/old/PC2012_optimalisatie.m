...
ret_cats = InPortefeuille==2; % welke assets zitten in de return portfolio ?

%Verwerk restricties op portefeuillegewichten 
A   = [-eye(Nassets);eye(Nassets)];     
b   = [-Rest_tot(:,1);Rest_tot(:,2)]; % min / max gewichten.
...
%Pas scenariokansen aan -> zorg dat kansen altijd optellen tot 1.0
for i=1:scenarios
    p = ScenarioKans(i,:);
    if sum(p)>1
